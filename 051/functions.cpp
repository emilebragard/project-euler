...
void find_combos(int size, vector<int> potential_digits, vector<int> currentnum, vector<vector<int>>& combos, bool allow_repeats){
    ...
    for (int i = 0; i < potential_digits.size(); i++){
        ...
        if (new_num.size() == size ){
            if (new_num[0] != 0 && new_num.back() != 0 && new_num.back() % 2 == 1){
                // Ensure that each number pushed back has at least 2 of a number;
                bool has_double = false;
                for (int i = 0; i < 4; i ++){
                    int double_count = count(new_num.begin(), new_num.end(), i);
                    if (double_count >=2){has_double = true;}
                    }
                if (has_double){
